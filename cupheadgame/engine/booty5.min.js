...
b5.Actor.prototype.getFixtureSelf = function(e,t,o,n) {
	var a = function(r) {
		switch(r) {
			case b5.Actor.Type_Image:
			case b5.Actor.Type_Rect:
	...
	return {
		...
		friction: t || 1,
		restitution: o || 0,
		is_sensor: this._bodyInitType === 'collider' || n
	};
},
b5.Actor.prototype.addFixtureSelf = function(e,t,o,n) {
	this.addFixture(this.getFixtureSelf(e,t,o,n));
},
...
b5.Actor.prototype.disableCollisionMask = function(e) {
	var t = new Box2D.Dynamics.b2FilterData;
	t.maskBits = Math.pow(2,Math.trunc(Math.random()*16));
	this.getFixtures()[0].SetFilterData(t);
	e.getFixtures()[0].SetFilterData(t);
},
b5.Actor.prototype.releaseCollisionMask = function(e) {
	var t = new Box2D.Dynamics.b2FilterData;
	this.getFixtures()[0].SetFilterData(t);
	e.getFixtures()[0].SetFilterData(t);
},
...
*/
  ...
  b5.Bitmap = function(e, t, o, a) {
	...
	i && (
		...
		i.preserveBitmap = !0,
		i.bitmapFromImage = !1,//(b5.app||{}).low_memory_texture_decoding,
		i.fullAsync = (b5.app||{}).async_texture_decoding,
		i.resolution = r || 1
  );
...
b5.Bitmap.prototype.load = function() {
 ...
 // e.resource._onLoad$1 = () => {
		if(!e.loaded) {
			e.loaded_internal = !0;
			e.resolution = e.resource.resolution,
...
'use strict',
/*
* Collision detection using Aligned Axis Bounding Box
* It's way faster than SAT but doesn't support rotation
* usually used more	for circles and capsules
*/
b5.AABB = {
...
'use strict',
/*
* Collision detection using Separating Axis Theorem for transformed polygons
* Adapted by AndrewDev
* Huge thanks to https://www.sevenson.com.au/programming/sat/
* I did some modifications and optimizations to the PolyTestPoly method,
* it's less accurate but it's way faster and supports full rotation.
* It's enough for detecting rotated rectangles and circles
*/
b5.SAT = {
	vectorDotProduct: function(p1,p2) {
		return (p1.x*p2.x) + (p1.y*p2.y);
	},
	transformPoints: function(x, y, points, rotation) {
		for (var i = 0, verts = []; i < points.length; i += 2) {
			if(rotation != 0) {
				var a = Math.atan2(points[i+1], points[i])+rotation,
				l = Math.sqrt((points[i]*points[i])+(points[i+1]*points[i+1]));
			...
			else verts.push({x: x + points[i], y: y + points[i+1]});
		}
		return verts;
	},
	PolyTestPoly: function(pol1, pol2, pol1Rot, pol2Rot) {
  	var deg90rad = 1.5707963267948966, //90* (Math.PI/180)
  	rotation = pol1Rot || pol2Rot,
  	axes = [{
  		x: -1,
  		y: 0 //Horizontal axis
  	}, {
  		...
  		x: Math.cos(rotation),
  		y: Math.sin(rotation) //Horizontal rotation axis
  	}, {
  		x: Math.cos(rotation+deg90rad),
  		y: Math.sin(rotation+deg90rad) //Vertical rotation axis
  	}];
  	
  	//Project polygons on the axes
  	
  	for(var a = 0;a < axes.length; a++) {
  		//Get min/max interval for pol1
		 	var pol1min = b5.SAT.vectorDotProduct(axes[a], pol1[0]),
  		pol1max = pol1min;
  		
			for(var i = 1; i < pol1.length; i++) {
	 			var dot = b5.SAT.vectorDotProduct(axes[a], pol1[i]);
  			pol1min = Math.min(pol1min, dot);
  			pol1max = Math.max(pol1max, dot);
  		}
  	
  		//Get min/max interval for pol2
  		var pol2min = b5.SAT.vectorDotProduct(axes[a], pol2[0]),
  		pol2max = pol2min;
  	
  		for(var i = 1; i < pol2.length; i++) {
  			var dot = b5.SAT.vectorDotProduct(axes[a], pol2[i]);
  			pol2min = Math.min(pol2min, dot);
  			pol2max = Math.max(pol2max, dot);
  		}
  	
  		//Offsets are not neccessary since points are already transformed
    
    	//Overlap test, if there's a gap in any of the axes, then polygons are not overlaping
  		if(pol1min - pol2max > 0 || pol2min - pol1max > 0) return false;
  	}
  	return true;
  },
  PolyTestCircle: function(pol1, rotation, cir1Pos, radius) {
  	var deg90rad = 1.5707963267948966, 
  	axes = [{
  		x: -1,
  		y: 0
  	}, {
  	...
  	//Project polygon and circle on the axes
  	for(var a = 0; a < axes.length; a++) {
  		//Get min/max for pol1
		 	var pol1min = b5.Maths.vecDotProd(axes[a], pol1[0]),
  		pol1max = pol1min;
  		
			for(var i = 1; i < pol1.length; i++) {
	 			var dot = b5.Maths.vecDotProd(axes[a], pol1[i]);
  			pol1min = Math.min(pol1min, dot);
  			pol1max = Math.max(pol1max, dot);
  		}
  		
  		//Get min/max for circle
  		var dot = b5.Maths.vecDotProd(axes[a], cir1Pos);
  		cir1min = dot - radius;
  		cir1max = dot + radius;
  		
  		//Overlap test
  		if(pol1min - cir1max > 0 || cir1min - pol1max > 0) return false;
  	}
  	return true;
  }
  //CircleTestCircle can be done without SAT
},
