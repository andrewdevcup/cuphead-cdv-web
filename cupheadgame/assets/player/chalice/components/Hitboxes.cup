return function(thisPlayer, flags, input, world, scene, game, data) {
  ...
  var hb = new b5.ArcActor();
    ...
    if(last_anim != atlas.current_anim || shootAngle != thisPlayer.data.shootAngle || (descend != descending && (descend = descending, descending))) {
      ...
      //Special aim-in-air animations for Chalice
      //Use shooting angles when in first jump to use in air
      if(flags.inAir && !flags.inDoubleJump && !flags.shootingEX) switch(shootAngle * b5.Maths.RtoDeg) {
      	case 0:
      	case 180:
      		ca = "shooting_straight";
      		break;
      	case -90:
      		ca = "shooting_up";
      		break;
      ...
      switch(ca) {
      	...
      	case 'ex_straight_air':
      		thisPlayer.data.shootOffset = [93*facing, -10];
        	break;
        case 'shooting_up':
        case 'shooting_up_boil':
        	thisPlayer.data.shootOffset = [28*facing, -108];
        	break;
        case 'shooting_diag_up':
        case 'shooting_diag_up_boil':
        case 'ex_diag_up_ground':
      	case 'ex_diag_up_air':
        	thisPlayer.data.shootOffset = [75*facing, -59];
        	break;
        case 'shooting_diag_down':
        case 'shooting_diag_down_boil':
       	case 'ex_diag_down_ground':
      	case 'ex_diag_down_air':
        	thisPlayer.data.shootOffset = [70*facing, 30];
        	break;
        case 'shooting_down':
        case 'shooting_down_boil':
        	thisPlayer.data.shootOffset = [33*facing, 57];
        	break;
        case 'shooting_duck':
        case 'shooting_duck_boil':
        	thisPlayer.data.shootOffset = [96*facing, 18];
        	break;
        case 'shooting_run':
          thisPlayer.data.shootOffset = [72*facing, -8];
          break;
        case 'shooting_run_diag':
        	thisPlayer.data.shootOffset = [84*facing, -58];
        	break;
        case 'jump':
       	...
       	case 'ex_up_air':
        case 'ex_up_ground':
        	thisPlayer.data.shootOffset = [0, -140];
        	break;
        case 'ex_down_air':
        case 'ex_down_ground':
        	thisPlayer.data.shootOffset = [0, 35];
        	break;
      }
