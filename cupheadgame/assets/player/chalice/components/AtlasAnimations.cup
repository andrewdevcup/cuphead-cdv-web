return function(thisPlayer, flags, input, world, scene, game, data) {
	...
	thisPlayer.data.triggerShot = function() {
		...
		this.headTriggerShot = true;
	}
	
	var AtlasPlayAnim = function(a,r) {
		atlas.playAnim(a,r);
	}

	return (thisPlayer.updateAnims = function(thisPlayer, flags, input, world, scene, game, data) {
		...
		if (thisPlayer.updateAnimations) {
			...
			if (!flags.running && !flags.ducking && !flags.locking && !flags.dashing && !flags.inAir)
				flags.shooting ? vars.animTriggerShot ? playAnimAfter('shooting_straight', 'shooting_straight_boil'): AtlasPlayAnim('shooting_straight_boil', true)
				: !axis_y && (
					prevAimFrame=atlas.current_frame, 
					AtlasPlayAnim("idle_stand", true),
					atlas.current_frame = prevAimFrame
				);
     
     //Dashing and rolling animation
			if (flags.dashing) {
				atlas.current_anim != "dash" && AtlasPlayAnim( flags.ducking ? 'roll':"dash"); return
			}
      ...
      //Ducking state changed?
			...
			if (flags.ducking) {
				flipped ? playAnimAfter("duck_tr_flip", flags.shooting ? 'shooting_duck_boil': "duck", function() {
					thisPlayer.updateAnimations = true;
					flipchange = true;
				}): playAnimAfter(duckchange ? (flags.shooting ? 'duck_tr_shoot' : "duck_tr") : flags.shooting?'shooting_duck_boil': !flipchange && !shootchange && !flags.rolling ?'duck_tr':'duck', flags.shooting ? 'shooting_duck_boil': "duck");
			...
			if (!flags.inAir && !flags.dashing) {
				...
				if (flags.running) {
					...
					/*Running normal*/	if (!flags.shooting) flipped ? thisPlayer.setTimeout(()=>{playAnimAfter("run_flip", "run")},app.dt): AtlasPlayAnim("run", true);
					/*Running shoot diagonal*/  else if (flags.shootingDiagonal) flipped ? thisPlayer.setTimeout(()=>{playAnimAfter("shooting_run_diag_flip", "shooting_run_diag")},app.dt): AtlasPlayAnim("shooting_run_diag", true);
					/*Running shoot straight*/  else flipped ? thisPlayer.setTimeout(()=>{playAnimAfter("shooting_run_flip", "shooting_run")},app.dt): AtlasPlayAnim("shooting_run", true);
					...
					!vars.animTriggerShot ? (
						prevAimFrame = atlas.current_frame,
						AtlasPlayAnim(flags.shooting ? "shooting_up_boil" : "aim_up", true),
						!flags.shooting && (atlas.current_frame = prevAimFrame)
					)
					  : flags.shooting ? playAnimAfter('shooting_up', 'shooting_up_boil') : AtlasPlayAnim('aim_up',true);
				}

			} else if (flags.inDoubleJump) AtlasPlayAnim("jump", true);
			else if(!flags.parried) {
				atlas.onAnimEnd = null;
				prevFallFrame = atlas.current_frame;
				!flags.jumpActioned ? (
				  AtlasPlayAnim( flags.shooting ? 'fall_body':'fall')
				  )
				  : flags.shooting? AtlasPlayAnim('fall_body')
				  : AtlasPlayAnim('jump_fall');//playAnimAfter(thisPlayer.body.m_linearVelocity.y < 0 ?"jump_ground":'fall', 'fall');
				 atlas.current_frame = prevFallFrame;
			}
      //Reset triggerShot
			vars.animTriggerShot = false;
			
		}
	}, thisPlayer.updateAnims);
};