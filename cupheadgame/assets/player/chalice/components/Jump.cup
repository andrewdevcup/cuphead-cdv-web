return function(thisPlayer, flags, input, world, scene, game, data) {
	...
	thisPlayer.addDblJumpParticles = function(axis_x) {
		var scale = 1.15,
		s = new b5.Actor();
		s.atlas = scene.findResource('ChaliceFXAtlas','brush');
		s.playAnimWithEnd('dbl_jump_fx_back', f => f.destroy());
		thisPlayer.parent.addActor(s);
		s.setPosition(thisPlayer.x,thisPlayer.y);
		s.layer = thisPlayer.layer -1;
		s.setScale(
			thisPlayer.Sprite.scale_x * scale,
			thisPlayer.Sprite.scale_y * scale
		);
		
		var g = new b5.Actor();
		g.atlas = scene.findResource('ChaliceFXAtlas','brush');
		g.playAnimWithEnd('dbl_jump_fx_front', f => f.destroy());
		thisPlayer.parent.addActor(g);
		g.setPosition(thisPlayer.x,thisPlayer.y);
		g.layer = thisPlayer.layer + 1;
		g.oy = 30;
		g.setScale(
			thisPlayer.Sprite.scale_x * 0.85 * scale,
			thisPlayer.Sprite.scale_y * 0.85 * scale
		);
		
		axis_x > 0 ? s.rotation = g.rotation = 30 * b5.Maths.DtoRad
		: axis_x < 0 && (s.rotation = g.rotation = -30 * b5.Maths.DtoRad);
	...
	return function(thisPlayer, flags, input, world, scene, game, data) {
		...
		for (var i = 0; i < overlaps.length; i++) if (Math.round(thisPlayer.body.m_linearVelocity.y) >= 0) {
			if ((overlaps[i].isFloor || overlaps[i].isWall/*tags.indexOf('floor') != -1*/) && thisPlayer.y < overlaps[i].y - (overlaps[i].h/2)) {
	...
	/*	if (overlaps.length == 0) {
		...
		if (flags.inAir != jumping) {
			...
			if (jumping && flags.jumpState == 0) {
				...
				flags.ducking = false;
				!flags.shooting && !flags.dashing && thisPlayer.Sprite.playAnim('jump_ground');
			} else if (!jumping) {
				...
				//Update animations in this frame to sync with headSprite
				thisPlayer.updateAnimations = true;
				thisPlayer.updateAnims(thisPlayer, flags, input, world, scene, game, data);
		...
		if (input.A && !stA && flags.controllable) {
			...
			//	flags.parryActive = true;
			  ...
			  //Add particles
			  thisPlayer.addDblJumpParticles(input.axis_x);
				thisPlayer.updateAnimations = true;
